 ...
 * FULLY SIMULATED — no backend calls whatsoever
 * All data generated by drift engine + anomaly injection
 */
...
function generateDriftReading(drift: ReturnType<typeof createDriftState>, slNo: number, isAnomaly: boolean, source: Row["source"] = "stream"): Row {
  ...
  return { slNo, time: new Date().toLocaleTimeString(), ph, turbidity, tds, source };
}

/* ── FAKE PUMP DELAY ── */
type PumpCommand = "START_PUMP_A" | "START_PUMP_B" | "START_PUMP_C" | "STOP_ALL";
const FAKE_PUMP_DELAY = 600;
...
export default function LiveDashboard() {
  ...
  // Separate drift state for simulation/live modes so they don't share with idle stream
  const simDriftRef  = useRef(createDriftState());
  const slCounterRef = useRef(loadCounter());
  const [bufferSize, setBufferSize] = useState(0);
  const [rows,       setRows]       = useState<Row[]>([]);
  const [sparkBuffer, setSparkBuffer] = useState<Row[]>([]);
  ...
  /* ── IDLE STREAM — always-on background data every 3.5s ── */
  useEffect(() => {
    if (mode !== "idle") return;
    const tick = () => {
      const isAnomaly = Math.random() < ANOMALY_PROB;
      const row = generateDriftReading(driftRef.current, slCounterRef.current++, isAnomaly, "stream");
      ...
      setSparkBuffer(bufferRef.current.slice(-SPARKLINE_POINTS));
    };
    tick();
  ...
  /* ── PUMP — fully simulated with fake delay ── */
  const sendPumpCommand = useCallback((command: PumpCommand) => {
    setPumpBusy(true);
    setTimeout(() => {
  ...
  const startSimulation = useCallback(() => {
    simDriftRef.current = createDriftState();
    setMode("simulation");
  ...
  /* ── LIVE START — simulated with 800ms "deploy" delay ── */
  const startLiveSession = useCallback(() => {
    setRows([]); setPrediction(null); setReadyToSave(false);
    addToast("Deploying sensors…", "info");
    simDriftRef.current = createDriftState();
    setTimeout(() => {
      setMode("live");
      setSessionStatus({ active: true, completed: false, collected: 0, phase: "COLLECTING" });
  ...
  /* ── SESSION DATA GEN (simulation + live both use drift engine) ── */
  useEffect(() => {
    if (mode !== "simulation" && mode !== "live") return;
    if (rows.length >= MAX_ROWS) return;
    const id = setInterval(() => {
      setRows((prev) => {
        if (prev.length >= MAX_ROWS) return prev;
        const isAnomaly = Math.random() < ANOMALY_PROB;
        const newRow = generateDriftReading(
          simDriftRef.current,
          slCounterRef.current++,
          isAnomaly,
          mode === "live" ? "live" : "simulation",
        );
        saveCounter(slCounterRef.current);
        const updated = [...prev, newRow];
        bufferRef.current = [...bufferRef.current, newRow].slice(-ROLLING_BUFFER_MAX);
        saveBuffer(bufferRef.current);
        setBufferSize(bufferRef.current.length);
        setSparkBuffer(bufferRef.current.slice(-SPARKLINE_POINTS));
  ...
  /* ── RUN PREDICTION — pure simulation with fake 900ms delay ── */
  const runPrediction = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setSessionStatus((prev) => prev
        ? { ...prev, phase: "ANALYZED" }
        : { active: true, completed: true, collected: targetRows.length, phase: "ANALYZED" }
      );
      addToast(`Prediction: ${result.bracket} → Tank ${result.suggestedTank}`, "success");
      setPredicting(false);
    }, 900);
  }, [rows, predicting, addToast]);
  ...
  const saveIteration = useCallback(() => {
    ...
    if (mode !== "idle") returnToIdle();
  }, [iterationName, mode, rows, avg, prediction, iterations, addToast, returnToIdle]);
      ...
      <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            {/* Simulated badge — always show so it's clear no real backend is hit */}
            <span style={{ padding: "3px 8px", borderRadius: 8, fontSize: 10, fontWeight: 700, background: "#1e1b4b", color: "#a78bfa", border: "1px solid #4c1d95", letterSpacing: "0.08em" }}>SIM</span>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            {mode !== "idle" && (
              <button style={{ ...S.dangerBtn, padding: "10px 16px", fontSize: 13 }} onClick={returnToIdle}>✕ End Session</button>
            )}
            ...
            <button style={{ ...S.secondaryBtn }} onClick={startLiveSession}>Deploy Live Sensors</button>
          </div>
        </div>

        {/* SPARKLINE METRIC CARDS */}
        <div style={{ display: "flex", gap: 16, marginBottom: 24, flexWrap: "wrap" }}>
          <SparklineCard title="pH" valueKey="ph" buffer={sparkBuffer} color="#38bdf8" unit="" thresholdLow={6.5} thresholdHigh={8.5} onClick={() => sparkBuffer.length && setActiveMetric("ph")} />
          <SparklineCard title="Turbidity" valueKey="turbidity" buffer={sparkBuffer} color="#fbbf24" unit="NTU" thresholdHigh={8} onClick={() => sparkBuffer.length && setActiveMetric("turbidity")} />
          <SparklineCard title="TDS" valueKey="tds" buffer={sparkBuffer} color="#22c55e" unit="mg/L" thresholdHigh={800} onClick={() => sparkBuffer.length && setActiveMetric("tds")} />
        ...
        {sessionStatus && (
          <p style={{ marginTop: 8, color: "#94a3b8", fontSize: 13 }}>
            Phase: <b style={{ color: mode === "live" ? "#22c55e" : "#38bdf8" }}>{sessionStatus.phase}</b>
            <span style={{ marginLeft: 8, color: "#4b5563", fontSize: 11 }}>(simulated)</span>
          </p>
        )}
